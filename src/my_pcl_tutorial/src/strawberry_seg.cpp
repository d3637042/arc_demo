...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  if(!detected)
  {
    ...
    */
    ...
    // Publish strawberry frame
    tf::TransformBroadcaster br;
    tf::Transform transform;
    transform.setOrigin(tf::Vector3(cut_x, cut_y, cut_z));
    tf::Quaternion q;
    q.setRPY(0, 0, 0);
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), cloud_msg->header.frame_id, "strawberry"));
  ...
  q.setRPY(0, 0, 0);
  transform.setRotation(q);
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_rgb_optical_frame", "strawberry"));
  //std::cout << "TF send." << std::endl;
